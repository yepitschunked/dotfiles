...
(doc/"div.example_group").each do |example|
	puts "[#{(example/"dl/dt").inner_html}]"
	(example/"dd").each do |dd|
		txt = (dd/"span:first").inner_html
		puts "#{classes[dd[:class]]} #{txt}"
		next if dd[:class]!="example failed"
		failure  = (dd/"div.failure")
		msg		= (failure/"div.message/pre").inner_html
		back		= (failure/"div.backtrace/pre").inner_html
		ruby		= (failure/"pre.ruby/code").inner_html.scan(/(<span class="linenum">)(\d+)(<\/span>)([^<]+)/).map {|elem| "  "+elem[1]+": "+elem[3].chomp+"\n"}.join
		puts "  #{msg}"
		puts "  #{back}"
		puts ruby
	end
